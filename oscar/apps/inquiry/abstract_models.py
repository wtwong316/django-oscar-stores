...
from oscar.apps.inquiry.signals import (
    inquiry_line_status_changed, inquiry_status_changed)
...
logger = logging.getLogger('oscar.inquiry')


class AbstractInquiry(models.Model):
    """
    The main inquiry model
    """
    number = models.CharField(
        _("Inquiry number"), max_length=128, db_index=True, unique=True)

    # We track the site that each inquiry is placed within
    ...
    # Inquiries can be placed without the user authenticating so we don't always
    # have a renter ID.
    user = models.ForeignKey(
        AUTH_USER_MODEL, related_name='inquiries', null=True, blank=True,
        verbose_name=_("User"), on_delete=models.SET_NULL)

    # Billing address is not always required (eg paying by gift card)
    billing_address = models.ForeignKey(
        'inquiry.BillingAddress', null=True, blank=True,
    ...
    # inquiry-level charges are added and so we need to store it separately
    currency = models.CharField(
        _("Currency"), max_length=12, default=get_default_currency)
    total_incl_tax = models.DecimalField(
        _("Inquiry total (inc. tax)"), decimal_places=2, max_digits=12)
    total_excl_tax = models.DecimalField(
        _("Inquiry total (excl. tax)"), decimal_places=2, max_digits=12)
    ...
    shipping_address = models.ForeignKey(
        'inquiry.ShippingAddress', null=True, blank=True,
    ...
    # Use this field to indicate that an inquiry is on hold / awaiting payment
    ...
    #: Inquiry status pipeline.  This should be a dict where each (key, value) #:
    ...
    #: Inquiry status cascade pipeline.  This should be a dict where each (key,
    #: value) pair corresponds to an *inquiry* status and the corresponding
    #: *line* status that needs to be set when the inquiry is set to the new
    #: status
    ...
    def all_statuses(cls):
        """
        Return all possible statuses for an inquiry
        """
        return list(cls.pipeline.keys())

    def available_statuses(self):
        """
        Return all possible statuses that this inquiry can move to
        """
        return self.pipeline.get(self.status, ())

    def set_status(self, new_status):
        """
        Set a new status for this inquiry.

        If the inquiryed status is not valid, then ``InvalidInquiryStatus`` is
        raised.
        """
        ...
        if new_status not in self.available_statuses():
            raise exceptions.InvalidInquiryStatus(
                _("'%(new_status)s' is not a valid status for inquiry %(number)s"
        ...
        inquiry_status_changed.send(sender=self,
                                  inquiry=self,
                                  old_status=old_status,
                                  new_status=new_status,
                                  )

        self._create_inquiry_status_change(old_status, new_status)

    set_status.alters_data = True

    def _create_inquiry_status_change(self, old_status, new_status):
        # Not setting the status on the inquiry as that should be handled before
        self.status_changes.create(old_status=old_status, new_status=new_status)

    @property
    def is_anonymous(self):
        # It's possible for an inquiry to be placed by a renter who then
    ...
    def total_discount_incl_tax(self):
        """
        The amount of discount this inquiry received
        """
        discount = D('0.00')
        for line in self.lines.all():
    ...
    def num_items(self):
        """
        Returns the number of items in this inquiry.
        """
        num_items = 0
        for line in self.lines.all():
    ...
    def shipping_status(self):
        """Return the last complete shipping event for this inquiry."""
    ...
    def verification_hash(self):
        signer = Signer(salt='oscar.apps.inquiry.Inquiry')
        return signer.sign(self.number)

    def check_verification_hash(self, hash_to_check):
        """
        Checks the received verification hash against this inquiry number.
    ...
    def basket_discounts(self):
        # This includes both offer- and voucher- discounts.  For inquiries we
        # don't need to treat them differently like we do for baskets.
        return self.discounts.filter(
            category=AbstractInquiryDiscount.BASKET)

    @property
    def shipping_discounts(self):
        return self.discounts.filter(
            category=AbstractInquiryDiscount.SHIPPING)

    @property
    def post_inquiry_actions(self):
        return self.discounts.filter(
            category=AbstractInquiryDiscount.DEFERRED)
    ...
    def save(self, *args, **kwargs):
        ...
        # useful when importing inquiries from another system).
        self.set_date_placed_default()
        super().save(*args, **kwargs)


class AbstractInquiryNote(models.Model):
    """
    A note against an inquiry.

    This are often used for audit purposes too.  IE, whenever an admin
    makes a change to an inquiry, we create a note to record what happened.
    """
    ...
    class Meta:
        ...
        verbose_name = _("Inquiry Note")
        verbose_name_plural = _("Inquiry Notes")
...
class AbstractInquiryStatusChange(models.Model):
    inquiry = models.ForeignKey(
        'inquiry.Inquiry',
        on_delete=models.CASCADE,
        related_name='status_changes',
        verbose_name=_('Inquiry Status Changes')
    )
    ...
    class Meta:
        abstract = True
        app_label = 'inquiry'
        verbose_name = _('Inquiry Status Change')
        verbose_name_plural = _('Inquiry Status Changes')
        ordering = ['-date_created']

    def __str__(self):
        return _("%(inquiry)s has changed status from %(old_status)s to %(new_status)s") \
            % {'inquiry': self.inquiry, 'old_status': self.old_status, 'new_status': self.new_status, }


class AbstractCommunicationEvent(models.Model):
    """
    An inquiry-level event involving a communication to the renter, such
    ...
    def __str__(self):
        return _("'%(type)s' event for inquiry #%(number)s") \
            % {'type': self.event_type.name, 'number': self.inquiry.number}
...
class AbstractLine(models.Model):
    ...
    partner = models.ForeignKey(
        'partner.Partner', related_name='inquiry_lines', blank=True, null=True,
    ...
    class Meta:
        abstract = True
        app_label = 'inquiry'
        # Enforce sorting in inquiry of creation.
        ordering = ['pk']
        verbose_name = _("Inquiry Line")
    ...
    def all_statuses(cls):
        """
        Return all possible statuses for an inquiry line
        """
        return list(cls.pipeline.keys())

    def available_statuses(self):
        """
        Return all possible statuses that this inquiry line can move to
        """
        return self.pipeline.get(self.status, ())
    ...
    def set_status(self, new_status):
        ...
        If the inquiryed status is not valid, then ``InvalidLineStatus`` is
        raised.
        """
        if new_status == self.status:
            return
        ...
        # Send signal for handling status changed
        inquiry_line_status_changed.send(sender=self,
                                       line=self,
                                       old_status=old_status,
                                       new_status=new_status,
                                       )
...
class AbstractPaymentEvent(models.Model):
    """
    A payment event for an inquiry
    ...
    * 2 lines have been refunded
    ...
    lines = models.ManyToManyField(
        'inquiry.Line', through='PaymentEventQuantity',
        verbose_name=_("Lines"))
    event_type = models.ForeignKey(
        'inquiry.PaymentEventType',
        on_delete=models.CASCADE,
        verbose_name=_("Event Type"))
    ...
    def __str__(self):
        return _("Payment event for inquiry %s") % self.inquiry
...
class AbstractShippingEvent(models.Model):
    ...
    inquiry = models.ForeignKey(
        ...
        verbose_name=_("Inquiry"))
    lines = models.ManyToManyField(
        'inquiry.Line', related_name='shipping_events',
        through='ShippingEventQuantity', verbose_name=_("Lines"))
    event_type = models.ForeignKey(
        'inquiry.ShippingEventType',
        on_delete=models.CASCADE,
        verbose_name=_("Event Type"))
    ...
    def __str__(self):
        return _("Inquiry #%(number)s, type %(type)s") % {
            'number': self.inquiry.number,
...
class AbstractInquiryDiscount(models.Model):
    """
    A discount against an inquiry.

    Normally only used for display purposes so an inquiry can be listed with
    ...
    # Post-inquiry offer applications can return a message to indicate what
    # action was taken after the inquiry was placed.
    ...
    @property
    def is_post_inquiry_action(self):
    ...
    class Meta:
        ...
        app_label = 'inquiry'
        ordering = ['pk']
        verbose_name = _("Inquiry Discount")
        verbose_name_plural = _("Inquiry Discounts")
    ...
    def __str__(self):
        return _("Discount of %(amount)r from inquiry %(inquiry)s") % {
            'amount': self.amount, 'inquiry': self.inquiry}
