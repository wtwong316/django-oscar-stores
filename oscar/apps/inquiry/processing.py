...
from oscar.apps.inquiry import exceptions


class EventHandler(object):
    """
    Handle requested inquiry events.

    This is an important class: it houses the core logic of your shop's inquiry
    processing pipeline.
    """
    ...
    def handle_shipping_event(self, inquiry, event_type, lines,
                              line_quantities, **kwargs):
        """
        Handle a shipping event for a given inquiry.

        This is most common entry point to this class - most of your inquiry
        ...
        specifics of you inquiry processing pipeline.
        """
        # Example implementation
        self.validate_shipping_event(
            inquiry, event_type, lines, line_quantities, **kwargs)
        return self.create_shipping_event(
            inquiry, event_type, lines, line_quantities, **kwargs)

    def handle_payment_event(self, inquiry, event_type, amount, lines=None,
                             line_quantities=None, **kwargs):
        """
        Handle a payment event for a given inquiry.
        ...
        self.validate_payment_event(
            inquiry, event_type, amount, lines, line_quantities, **kwargs)
        return self.create_payment_event(
            inquiry, event_type, amount, lines, line_quantities, **kwargs)

    def handle_inquiry_status_change(self, inquiry, new_status, note_msg=None):
        """
        Handle a requested inquiry status change

        This method is not normally called directly by client code.  The main
        use-case is when an inquiry is cancelled, which in some ways could be
        viewed as a shipping event affecting all lines.
        """
        inquiry.set_status(new_status)
        if note_msg:
            self.create_note(inquiry, note_msg)

    # Validation methods
    # ------------------

    def validate_shipping_event(self, inquiry, event_type, lines,
                                line_quantities, **kwargs):
        """
        ...
        for line, qty in zip(lines, line_quantities):
            ...
            # 'is_shipping_event_permitted' and enforce the correct inquiry of
            # shipping events.
    ...
    def validate_payment_event(self, inquiry, event_type, amount, lines=None,
                               line_quantities=None, **kwargs):
    ...
    def have_lines_passed_shipping_event(self, inquiry, lines, line_quantities,
                                         event_type):
        """
    ...
    def calculate_payment_event_subtotal(self, event_type, lines,
        ...
        for line, qty_to_consume in zip(lines, line_quantities):
            ...
            # Consume prices in inquiry of ID (this is the default but it's
    ...
    def consume_stock_allocations(self, inquiry, lines=None, line_quantities=None):
        """
    ...
    def cancel_stock_allocations(self, inquiry, lines=None, line_quantities=None):
        """
    ...
    def create_shipping_event(self, inquiry, event_type, lines, line_quantities,
                              **kwargs):
        reference = kwargs.get('reference', '')
        event = inquiry.shipping_events.create(
    ...
    def create_payment_event(self, inquiry, event_type, amount, lines=None,
                             line_quantities=None, **kwargs):
        reference = kwargs.get('reference', "")
        event = inquiry.payment_events.create(
    ...
    def create_communication_event(self, inquiry, event_type):
        return inquiry.communication_events.create(event_type=event_type)

    def create_note(self, inquiry, message, note_type='System'):
        return inquiry.notes.create(
