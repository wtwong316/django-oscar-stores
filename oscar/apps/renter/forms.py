...
from oscar.apps.renter.utils import get_password_reset_url, normalise_email
...
class PasswordResetForm(auth_forms.PasswordResetForm):
    ...
    def send_password_reset_email(self, site, user, request=None):
        ...
        RenterDispatcher().send_password_reset_email_for_user(user, extra_context)
...
class InquirySearchForm(forms.Form):
    ...
    def clean(self):
        if self.is_valid() and not any([self.cleaned_data['date_from'],
                                        ...
                                        self.cleaned_data['inquiry_number']]):
            raise forms.ValidationError(_("At least one field is required."))
        return super().clean()

    def description(self):
        """
        Uses the form's data to build a useful description of what inquiries
        are listed.
        """
        if not self.is_bound or not self.is_valid():
            return _('All inquiries')
        else:
            date_from = self.cleaned_data['date_from']
            date_to = self.cleaned_data['date_to']
            inquiry_number = self.cleaned_data['inquiry_number']
            return self._inquiries_description(date_from, date_to, inquiry_number)

    def _inquiries_description(self, date_from, date_to, inquiry_number):
        if date_from and date_to:
            if inquiry_number:
                desc = _('Inquiries placed between %(date_from)s and '
                         '%(date_to)s and inquiry number containing '
                         '%(inquiry_number)s')
            else:
                desc = _('Inquiries placed between %(date_from)s and '
                         '%(date_to)s')
            ...
            if inquiry_number:
                desc = _('Inquiries placed since %(date_from)s and '
                         'inquiry number containing %(inquiry_number)s')
            else:
                desc = _('Inquiries placed since %(date_from)s')
        elif date_to:
            if inquiry_number:
                desc = _('Inquiries placed until %(date_to)s and '
                         'inquiry number containing %(inquiry_number)s')
            else:
                desc = _('Inquiries placed until %(date_to)s')
        elif inquiry_number:
            desc = _('Inquiries with inquiry number containing %(inquiry_number)s')
        else:
            return None
        params = {
            'date_from': date_from,
    ...
    def get_filters(self):
        ...
        if inquiry_number:
            kwargs['number__contains'] = inquiry_number
        return kwargs
...
if Profile:  # noqa (too complex (12))
    ...
    class UserAndProfileForm(forms.ModelForm):
        ...
        def __init__(self, user, *args, **kwargs):
            ...
            # Ensure inquiry of fields is email, user fields then profile fields
